...
namespace BitcoinNet.Scripting
{
	public class ScriptEvaluationContext
	{
		...
		private static readonly byte[] VchTrue = {1};

		private static readonly byte[] VchMaxModOrder =
		{
		...
		private static readonly byte[] VchMaxModHalfOrder =
		{
		...
		public ContextStack<byte[]> Stack { get; private set; } = new ContextStack<byte[]>();

		public ScriptVerify ScriptVerify { get; set; }

		public SigHash SigHash { get; set; }

		public ScriptError Error { get; set; }
		...
		public bool EvalScript(Script s, TransactionChecker checker)
		{
			...
			try
			{
				...
				while ((opcode = script.Read()) != null)
				{
					...
					// Read instruction
					//
					if (opcode.PushData != null && opcode.PushData.Length > MaxScriptElementSize)
					{
					...
					else if (fExec || OpcodeType.OP_IF <= opcode.Code && opcode.Code <= OpcodeType.OP_ENDIF)
					{
						switch (opcode.Code)
						{
							...
							case OpcodeType.OP_CHECKLOCKTIMEVERIFY:
							{
								...
								var nLockTime = new CScriptNum(Stack.Top(-1), fRequireMinimal, 5);
							...
							case OpcodeType.OP_CHECKSEQUENCEVERIFY:
							{
								...
								var nSequence = new CScriptNum(Stack.Top(-1), fRequireMinimal, 5);
							...
							case OpcodeType.OP_2DUP:
							{
								...
								var vch1 = Stack.Top(-2);
								var vch2 = Stack.Top(-1);
								Stack.Push(vch1);
								Stack.Push(vch2);
								break;
							}
							case OpcodeType.OP_3DUP:
							{
								...
								var vch1 = Stack.Top(-3);
								var vch2 = Stack.Top(-2);
								var vch3 = Stack.Top(-1);
								Stack.Push(vch1);
								Stack.Push(vch2);
								Stack.Push(vch3);
								break;
							...
							case OpcodeType.OP_2OVER:
							{
								...
								var vch1 = Stack.Top(-4);
								var vch2 = Stack.Top(-3);
								Stack.Push(vch1);
								Stack.Push(vch2);
								break;
							}
							case OpcodeType.OP_2ROT:
							{
								...
								var vch1 = Stack.Top(-6);
								var vch2 = Stack.Top(-5);
								Stack.Remove(-6, -4);
								Stack.Push(vch1);
								Stack.Push(vch2);
								break;
							...
							{
								// -- stacksize
								var bn = new CScriptNum(Stack.Count);
							...
							case OpcodeType.OP_ROLL:
							{
								...
								var n = new CScriptNum(Stack.Top(-1), fRequireMinimal).GetInt();
								Stack.Pop();
							...
							case OpcodeType.OP_SIZE:
							{
								...
								var bn = new CScriptNum(Stack.Top(-1).Length);
								Stack.Push(bn.GetVch());
								break;
							}
							//
							...
							case OpcodeType.OP_0NOTEQUAL:
							{
								...
								var bn = new CScriptNum(Stack.Top(-1), fRequireMinimal);
								switch (opcode.Code)
								{
									case OpcodeType.OP_1ADD:
										bn += 1;
							...
							case OpcodeType.OP_MAX:
							{
								...
								var bn1 = new CScriptNum(Stack.Top(-2), fRequireMinimal);
								var bn2 = new CScriptNum(Stack.Top(-1), fRequireMinimal);
							...
							case OpcodeType.OP_WITHIN:
							{
								...
								var bn1 = new CScriptNum(Stack.Top(-3), fRequireMinimal);
								var bn2 = new CScriptNum(Stack.Top(-2), fRequireMinimal);
								var bn3 = new CScriptNum(Stack.Top(-1), fRequireMinimal);
							...
							case OpcodeType.OP_HASH256:
							{
								...
								byte[]
									vchHash =
										null; //((opcode == OpcodeType.OP_RIPEMD160 || opcode == OpcodeType.OP_SHA1 || opcode == OpcodeType.OP_HASH160) ? 20 : 32);
							...
							case OpcodeType.OP_CHECKSIGVERIFY:
							{
								...
								var vchSig = Stack.Top(-2);
								var vchPubKey = Stack.Top(-1);
								...
								var fSuccess = CheckSig(vchSig, vchPubKey, scriptCode, checker);
							...
							case OpcodeType.OP_CHECKMULTISIGVERIFY:
							{
								...
								var nKeysCount = new CScriptNum(Stack.Top(-i), fRequireMinimal).GetInt();
								...
								var nSigsCount = new CScriptNum(Stack.Top(-i), fRequireMinimal).GetInt();
								...
								{
									var vchSig = Stack.Top(-isig);
									var vchPubKey = Stack.Top(-ikey);
									...
									var fOk = CheckSig(vchSig, vchPubKey, scriptCode, checker);
								...
								while (i-- > 1)
								{
									...
									if (!fSuccess && (ScriptVerify & ScriptVerify.NullFail) != 0 && ikey2 == 0 &&
								...
								if ((ScriptVerify & ScriptVerify.NullDummy) != 0 && Stack.Top(-1).Length != 0)
								{
		...
		private bool CheckSequence(CScriptNum nSequence, TransactionChecker checker)
		{
			...
			var txToSequence = txTo.Inputs[nIn].Sequence;
			...
			if (!(
				txToSequenceMasked < Sequence.SEQUENCE_LOCKTIME_TYPE_FLAG &&
				nSequenceMasked < Sequence.SEQUENCE_LOCKTIME_TYPE_FLAG ||
				txToSequenceMasked >= Sequence.SEQUENCE_LOCKTIME_TYPE_FLAG &&
				nSequenceMasked >= Sequence.SEQUENCE_LOCKTIME_TYPE_FLAG
			))
			{
		...
		private bool CheckLockTime(CScriptNum nLockTime, TransactionChecker checker)
		{
			...
			if (!(
				txTo.LockTime < LockTime.LockTimeThreshold && nLockTime < LockTime.LockTimeThreshold ||
				txTo.LockTime >= LockTime.LockTimeThreshold && nLockTime >= LockTime.LockTimeThreshold
			))
			{
				return false;
		...
		internal bool CheckSignatureEncoding(byte[] vchSig)
		{
			...
			if ((ScriptVerify & (ScriptVerify.DerSig | ScriptVerify.LowS | ScriptVerify.StrictEnc)) != 0 &&
		...
		public bool CheckSig(TransactionSignature signature, PubKey pubKey, Script scriptPubKey, Transaction txTo,
			uint nIn)
		{
			return CheckSig(signature.ToBytes(), pubKey.ToBytes(), scriptPubKey, txTo, (int) nIn);
		}

		public bool CheckSig(TransactionSignature signature, PubKey pubKey, Script scriptPubKey,
		...
		private bool CheckSig(byte[] vchSig, byte[] vchPubKey, Script scriptCode, TransactionChecker checker)
		{
			PubKey pubkey = null;
			try
			{
			...
			var sighash = checker.Transaction.GetSignatureHash(scriptCode, checker.Index, scriptSig.SigHash,
				checker.Amount, checker.PrecomputedTransactionData);
		...
		{
			Stack = new ContextStack<byte[]>(other.Stack);
		...
		public ScriptEvaluationContext Clone()
		{
			...
			{
				Stack = new ContextStack<byte[]>(Stack),
		...
		private class CScriptNum
		{
			...
			{
				return new CScriptNum(a._mValue & b);
			}
			...
			{
				Assert(num._mValue != long.MinValue);
			...
			private static long SetVch(byte[] vch)
			{
				if (vch.Length == 0)
				{
					return 0;
