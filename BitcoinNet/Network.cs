...
namespace BitcoinNet
{
	...
	public partial class Network
	{
		...
		static Network()
		{
			_Main = new Network();
			_Main.InitMain();
			_Main._NetworkSet = BitcoinNet.BitcoinCash.Instance;
			_Main.Consensus.Freeze();

			_TestNet = new Network();
			_TestNet._NetworkSet = BitcoinNet.BitcoinCash.Instance;
			_TestNet.InitTest();
			_TestNet.Consensus.Freeze();

			_RegTest = new Network();
			_RegTest._NetworkSet = BitcoinNet.BitcoinCash.Instance;
			_RegTest.InitReg();
		...
		internal static Network Register(NetworkBuilder builder)
		{
			...
			network.Prefix = builder._prefix;
		...
		private void InitMain()
		{
			...
			consensus.MinimumChainWork = new uint256("0000000000000000000000000000000000000000007e5dbf54c7f6b58a6853cd");
		...
		private void InitTest()
		{
			...
			consensus.PowLimit = new Target(new uint256("00000000ffffffffffffffffffffffffffffffffffffffffffffffffffffffff"));
			consensus.MinimumChainWork = new uint256("00000000000000000000000000000000000000000000002888c34d61b53a244a");
			consensus.PowTargetTimespan = TimeSpan.FromSeconds(14 * 24 * 60 * 60); // two weeks
			consensus.PowTargetSpacing = TimeSpan.FromSeconds(10 * 60);
			...
			vSeeds.Clear();
			vSeeds.Add(new DNSSeedData("bitcoinabc.org", "testnet-seed.bitcoinabc.org"));
			vSeeds.Add(new DNSSeedData("bitcoinforks.org", "testnet-seed-abc.bitcoinforks.org"));
		...
		private void InitReg()
		{
			...
			_GenesisBytes = Encoders.Hex.DecodeData("");
			consensus.SetBlock(_GenesisBytes);
			nDefaultPort = 18444;
			nRPCPort = 18443;
			//strDataDir = "regtest";
