...
namespace BitcoinNet.Crypto
{
	internal class ECKey
	{
		...
		static ECKey()
		{
			_Secp256k1 = BitcoinNet.BouncyCastle.Crypto.EC.CustomNamedCurves.Secp256k1;
		...
		{
			if(isPrivate)
				_Key = new ECPrivateKeyParameters(new BitcoinNet.BouncyCastle.Math.BigInteger(1, vch), DomainParameter);
			else
			{
		...
		public static ECKey RecoverFromSignature(int recId, ECDSASignature sig, uint256 message, bool compressed)
		{
			...
			var n = curve.N;
			var i = BitcoinNet.BouncyCastle.Math.BigInteger.ValueOf((long)recId / 2);
			...
			var e = new BitcoinNet.BouncyCastle.Math.BigInteger(1, message.ToBytes());
			...
			var eInv = BitcoinNet.BouncyCastle.Math.BigInteger.Zero.Subtract(e).Mod(n);
		...
		private static ECPoint DecompressKey(BitcoinNet.BouncyCastle.Math.BigInteger xBN, bool yBit)
		{
