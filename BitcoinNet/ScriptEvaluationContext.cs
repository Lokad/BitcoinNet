...
namespace BitcoinNet
{
	...
	public class ScriptEvaluationContext
	{
		...
		public ScriptEvaluationContext()
		{
			...
			SigHash = BitcoinNet.SigHash.Undefined;
...
0xDF,0xE9,0x2F,0x46,0x68,0x1B,0x20,0xA0
		...
		bool CheckSig(byte[] vchSig, byte[] vchPubKey, Script scriptCode, TransactionChecker checker, int sigversion)
		{
			...
			if(!pubkey.Verify(sighash, scriptSig.Signature))
			{
				...
				var R = 4;
				var S = 6 + nLenR;
				var newS = new BitcoinNet.BouncyCastle.Math.BigInteger(1, vchSig, S, nLenS);
				var newR = new BitcoinNet.BouncyCastle.Math.BigInteger(1, vchSig, R, nLenR);
		...
		{
			if(SigHash == BitcoinNet.SigHash.Undefined)
				return true;
