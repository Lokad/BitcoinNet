...
namespace BitcoinNet
{
	...
	public struct Sequence
	{
		...
		private readonly uint _ValueInv;
		...
		public Sequence(int lockHeight)
		{
			...
			{
				throw new ArgumentOutOfRangeException(nameof(lockHeight),
					"Relative lock height must be positive and lower or equals to 0xFFFF (65535 blocks)");
			}
		...
		public Sequence(TimeSpan period)
		{
			if (period.TotalSeconds > 0xFFFF * 512 || period.TotalSeconds < 0)
			{
				throw new ArgumentOutOfRangeException(nameof(period),
					"Relative lock time must be positive and lower or equals to " +
					0xFFFF * 512 + " seconds (approx 388 days)");
			}

			var value = (uint) (period.TotalSeconds / (1 << SEQUENCE_LOCKTIME_GRANULARITY));
		...
		public bool IsRBF => Value < 0xffffffff - 1;
		...
		public TimeSpan LockPeriod
		{
			get
			{
				...
				return TimeSpan.FromSeconds((int) (Value & SEQUENCE_LOCKTIME_MASK) *
				                            (1 << SEQUENCE_LOCKTIME_GRANULARITY));
			}
		}
	}
}