...
namespace BitcoinNet.Tests
{
	public class NodeServerTester : IDisposable
	{
		...
		public NodeServerTester()
		{
			...
			while (true)
			{
				try
				{
					...
					Server1 = new NodeServer(network, internalPort: a);
					Server1.AllowLocalPeers = true;
					Server1.ExternalEndpoint = new IPEndPoint(IPAddress.Parse("127.0.0.1").MapToIPv6Ex(), a);
					Server1.Listen();
					Assert.True(Server1.IsListening);
					Server2 = new NodeServer(network, internalPort: b);
					Server2.AllowLocalPeers = true;
					Server2.ExternalEndpoint = new IPEndPoint(IPAddress.Parse("127.0.0.1").MapToIPv6Ex(), b);
					Server2.Listen();
					Assert.True(Server2.IsListening);
					break;
				}
				catch (Exception)
				{
		...
		public NodeServer Server1 { get; }

		public NodeServer Server2 { get; }
		...
		public Node Node1
		{
			get
			{
				...
				Thread.Sleep(
					0); //Don't underestimate thread preemption... without that the tests crash in mono proc... :(
				Thread.Sleep(0);
				return _node1;
			}
		}
...
#pragma warning restore CS4014
		...
		public void CanParseMessages()
		{
			...
			var tests = new[]
			{
				new
				{
					Version = 209U,
					Message =
						"e3e1f3e876657273696f6e0000000000550000009c7c00000100000000000000e615104d00000000010000000000000000000000000000000000ffff0a000001208d010000000000000000000000000000000000ffff0a000002208ddd9d202c3ab457130055810100",
				...
				new
				{
					Version = 60002U,
					Message =
						"e3e1f3e876657273696f6e000000000064000000358d493262ea0000010000000000000011b2d05000000000010000000000000000000000000000000000ffff000000000000000000000000000000000000000000000000ffff0000000000003b2eb35d8ce617650f2f5361746f7368693a302e372e322fc03e0300",
					Test = new Action<object>(o =>
					{
						var version = (VersionPayload) o;
				...
				{
					Version = 70012U,
					Message = "e3e1f3e876657261636b000000000000000000005df6e0e2",
					Test = new Action<object>(o =>
					{
				...
				new
				{
					Version = 60002U,
					Message =
						"e3e1f3e86164647200000000000000001f000000ed52399b01e215104d010000000000000000000000000000000000ffff0a000001208d",
					Test = new Action<object>(o =>
					{
		...
		{
			var hex =
				"f9beb4d972656a6563740000000000003a000000db7f7e7802747812156261642d74786e732d696e707574732d7370656e74577a9694da4ff41ae999f6591cff3749ad6a7db19435f3d8af5fecbcff824196";
			...
			Assert.True(
				reject.Hash == uint256.Parse("964182ffbcec5fafd8f33594b17d6aad4937ff1c59f699e91af44fda94967a57"));
		}
