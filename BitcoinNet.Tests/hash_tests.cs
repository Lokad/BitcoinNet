...
*/
		private readonly ulong[] siphash_4_2_testvec =
		{
		...
		public void hash256()
		{
			Assert.Equal(uint256.Parse("000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f"),
				Network.Main.GetGenesis().GetHash());
		}
		...
		public void siphash()
		{
			...
			byte[] t3 = {18, 19, 20, 21, 22, 23, 24, 25, 26};
			hasher.Write(t3);
			Assert.Equal(hasher.Finalize(), 0x2f2e6163076bcfadUL);
			byte[] t4 = {27, 28, 29, 30, 31};
			hasher.Write(t4);
			...
			Assert.Equal(
				Hashes.SipHash(0x0706050403020100UL, 0x0F0E0D0C0B0A0908UL,
