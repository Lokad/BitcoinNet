...
namespace BitcoinNet.Tests
{
	public class bip32_tests
	{
		...
		{
			public readonly string strHexMaster;
			public readonly List<TestDerivation> vDerive = new List<TestDerivation>();
		...
		private readonly TestVector test1 =
		...
		private readonly TestVector test2 =
			new TestVector(
		...
		public void CanRoundTripExtKeyBase58Data()
		{
			...
			Assert.True(ExtPubKey.Parse(pubkey.ToString(Network.Main)).ToString(Network.Main) ==
			            pubkey.ToString(Network.Main));
		}
		...
		public void CheckBIP32Constructors()
		{
			...
			Assert.Equal(key.Neuter().GetWif(Network.Main),
				new ExtPubKey(key.PrivateKey.PubKey, key.ChainCode).GetWif(Network.Main));

			key = key.Derive(1);
			Assert.Equal(key.GetWif(Network.Main),
				new ExtKey(key.PrivateKey, key.ChainCode, key.Depth, key.Fingerprint, key.Child).GetWif(Network.Main));
			Assert.Equal(key.Neuter().GetWif(Network.Main),
				new ExtPubKey(key.PrivateKey.PubKey, key.ChainCode, key.Depth, key.Fingerprint, key.Child).GetWif(
					Network.Main));
		}
	}
}