...
namespace BitcoinNet.Tests
{
	...
	{
		private static Dictionary<string, OpcodeType> _mapOpNames = new Dictionary<string, OpcodeType>();

		public static Script ParseScript(string s)
		{
			var result = new MemoryStream();
			if (_mapOpNames.Count == 0)
			{
				_mapOpNames = new Dictionary<string, OpcodeType>(Op.OpcodeByName);
				foreach (var kv in _mapOpNames.ToArray())
				{
					if (kv.Key.StartsWith("OP_", StringComparison.Ordinal))
					{
						var name = kv.Key.Substring(3, kv.Key.Length - 3);
						_mapOpNames.AddOrReplace(name, kv.Value);
					}
				}
			}
			...
			foreach (var w in words)
			{
				...
				else if (_mapOpNames.ContainsKey(w))
				{
					// opcode, e.g. OP_ADD or ADD:
					result.WriteByte((byte) _mapOpNames[w]);
				}
				else
				{
		...
		private void BIP65_testsCore(LockTime target, LockTime now, bool expectedResult)
		{
			...
			Assert.Equal(expectedResult,
				spending.Inputs.AsIndexedInputs().First().VerifyScript(tx.Outputs[0].ScriptPubKey));
		...
		private void AssertVerifyScript(Money amount, Script scriptSig, Script scriptPubKey, ScriptVerify flags,
			int testIndex, string comment, ScriptError expectedError)
		{
			...
			ScriptError actual;
			Script.VerifyScript(scriptSig, scriptPubKey, spendingTransaction, 0, amount, flags, SigHash.Undefined,
				out actual);
		...
		private void EnsureHasLibConsensus()
		{
			...
			File.Copy(libConsensusPath, Script.LibConsensusDll, false);
#endif
		}
		...
		{
			var hash = transaction.GetSignatureHash(scriptPubKey, 0, SigHash.All);
		...
		{
			return sign_multisig(scriptPubKey, new[] {key}, transaction);
		}
		...
		private void SignSignature(Key[] keys, Transaction txFrom, Transaction txTo, int n,
		...
		public void CanExtractAddressesFromScript()
		{
			var payToMultiSig =
				new Script(
					"1 0364bd4b02a752798342ed91c681a48793bb1c0853cbcd0b978c55e53485b8e27c 0364bd4b02a752798342ed91c681a48793bb1c0853cbcd0b978c55e53485b8e27d 2 OP_CHECKMULTISIG");
			...
			Assert.True(payToScriptHash.GetDestination().GetAddress(Network.Main).GetType() ==
			            typeof(BitcoinScriptAddress));

			var payToPubKeyHash =
				new Script("OP_DUP OP_HASH160 356facdac5f5bcae995d13e667bb5864fd1e7d59 OP_EQUALVERIFY OP_CHECKSIG");
			...
			Assert.True(payToPubKeyHash.GetDestination().GetAddress(Network.Main).GetType() ==
			            typeof(BitcoinPubKeyAddress));

			var p2shScriptSig = new Script(
				"0 3044022064f45a382a15d3eb5e7fe72076eec4ef0f56fde1adfd710866e729b9e5f3383d02202720a895914c69ab49359087364f06d337a2138305fbc19e20d18da78415ea9301 51210364bd4b02a752798342ed91c681a48793bb1c0853cbcd0b978c55e53485b8e27c210364bd4b02a752798342ed91c681a48793bb1c0853cbcd0b978c55e53485b8e27d52ae");
			...
			var p2phScriptSig =
				new Script(
					"3045022100af878a48aab5a71397d518ee1ae3c35267cb559240bc4a06926d65d575090e7f02202a9208e1f13683b4e450b349ae3e7bd4498d5d808f06c4b8059ea41595447af401 02a71e88db4924c7620f3b27fa748817444b6ad02cd8cea32ed3cf2deb8b5ccae7");
		...
		public void CanExtractScriptCode()
		{
			var script = new Script(
				"022b1300040df7414c0251433b7a2516e81689b02e33299c87ae870b5c9407b761 OP_DEPTH 3 OP_EQUAL OP_IF OP_SWAP 020524b8de0a1b57478f2d0e07aa9ea375b736f072281b3749fea044392bccfc52 OP_CHECKSIGVERIFY OP_CODESEPARATOR OP_CHECKSIG OP_ELSE 0 OP_CLTV OP_DROP OP_CHECKSIG OP_ENDIF");
			...
			Assert.Equal("OP_CHECKSIG OP_ELSE 0 OP_CLTV OP_DROP OP_CHECKSIG OP_ENDIF",
				script.ExtractScriptCode(0).ToString());
		...
		public void CanParseAndGeneratePayToMultiSig()
		{
			var scriptPubKey =
				"1 0364bd4b02a752798342ed91c681a48793bb1c0853cbcd0b978c55e53485b8e27c 0364bd4b02a752798342ed91c681a48793bb1c0853cbcd0b978c55e53485b8e27d 2 OP_CHECKMULTISIG";
			var scriptPubKeyResult =
				PayToMultiSigTemplate.Instance.ExtractScriptPubKeyParameters(new Script(scriptPubKey));
			Assert.Equal("0364bd4b02a752798342ed91c681a48793bb1c0853cbcd0b978c55e53485b8e27c",
				scriptPubKeyResult.PubKeys[0].ToString());
			Assert.Equal("0364bd4b02a752798342ed91c681a48793bb1c0853cbcd0b978c55e53485b8e27d",
				scriptPubKeyResult.PubKeys[1].ToString());
			Assert.Equal(1, scriptPubKeyResult.SignatureCount);
			Assert.Equal(scriptPubKey,
				PayToMultiSigTemplate.Instance.GenerateScriptPubKey(1, scriptPubKeyResult.PubKeys).ToString());

			var scriptSig =
				"0 3044022064f45a382a15d3eb5e7fe72076eec4ef0f56fde1adfd710866e729b9e5f3383d02202720a895914c69ab49359087364f06d337a2138305fbc19e20d18da78415ea9301 3044022064f45a382a15d3eb5e7fe72076eec4ef0f56fde1adfd710866e729b9e5f3383d02202720a895914c69ab49359087364f06d337a2138305fbc19e20d18da78415ea9302";
			...
			Assert.Equal(
				...
				Encoders.Hex.EncodeData(result[0].ToBytes()));
			Assert.Equal(
				...
				Encoders.Hex.EncodeData(result[1].ToBytes()));

			Assert.Equal(scriptSig, PayToMultiSigTemplate.Instance.GenerateScriptSig(result).ToString());

			scriptSig =
				"0 0 3044022064f45a382a15d3eb5e7fe72076eec4ef0f56fde1adfd710866e729b9e5f3383d02202720a895914c69ab49359087364f06d337a2138305fbc19e20d18da78415ea9302";
			...
			var sig = new TransactionSignature(Encoders.Hex.DecodeData(
				"3044022064f45a382a15d3eb5e7fe72076eec4ef0f56fde1adfd710866e729b9e5f3383d02202720a895914c69ab49359087364f06d337a2138305fbc19e20d18da78415ea9301"));
			var actual = PayToScriptHashTemplate.Instance.GenerateScriptSig(new[] {sig, sig}, new Script(scriptPubKey));
			var expected = new Script(
				"0 3044022064f45a382a15d3eb5e7fe72076eec4ef0f56fde1adfd710866e729b9e5f3383d02202720a895914c69ab49359087364f06d337a2138305fbc19e20d18da78415ea9301 3044022064f45a382a15d3eb5e7fe72076eec4ef0f56fde1adfd710866e729b9e5f3383d02202720a895914c69ab49359087364f06d337a2138305fbc19e20d18da78415ea9301 " +
		...
		{
			var scriptPubKey = "0364bd4b02a752798342ed91c681a48793bb1c0853cbcd0b978c55e53485b8e27c OP_CHECKSIG";
			...
			scriptPubKey =
				"0464bd4b02a752798342ed91c681a48793bb1c0853cbcd0b978c55e53485b8e27cff45c67d5f7be479215e9a27cea37afe1a00fa968ae3cbad128c9cee403844b7 OP_CHECKSIG";
			...
			scriptSig =
				"0044022064f45a382a15d3eb5e7fe72076eec4ef0f56fde1adfd710866e729b9e5f3383d02202720a895914c69ab49359087364f06d337a2138305fbc19e20d18da78415ea9301";
		...
		{
			var scriptPubKey =
				new Script("OP_DUP OP_HASH160 b72a6481ec2c2e65aa6bd9b42e213dce16fc6217 OP_EQUALVERIFY OP_CHECKSIG");
			...
			var scriptSig =
				new Script(
					"3044022064f45a382a15d3eb5e7fe72076eec4ef0f56fde1adfd710866e729b9e5f3383d02202720a895914c69ab49359087364f06d337a2138305fbc19e20d18da78415ea9301 0364bd4b02a752798342ed91c681a48793bb1c0853cbcd0b978c55e53485b8e27c");

			var sigResult = PayToPubkeyHashTemplate.Instance.ExtractScriptSigParameters(scriptSig);
			Assert.Equal(
				"3044022064f45a382a15d3eb5e7fe72076eec4ef0f56fde1adfd710866e729b9e5f3383d02202720a895914c69ab49359087364f06d337a2138305fbc19e20d18da78415ea9301",
				Encoders.Hex.EncodeData(sigResult.TransactionSignature.ToBytes()));
			Assert.Equal("0364bd4b02a752798342ed91c681a48793bb1c0853cbcd0b978c55e53485b8e27c",
				sigResult.PublicKey.ToString());

			Assert.Equal(
				PayToPubkeyHashTemplate.Instance.GenerateScriptSig(sigResult.TransactionSignature, sigResult.PublicKey)
					.ToString(), scriptSig.ToString());
			Assert.Equal(PayToPubkeyHashTemplate.Instance.GenerateScriptPubKey(pubKey).ToString(),
		...
		public void CanParseAndGeneratePayToScript()
		{
			...
			Assert.Equal(
				"3044022064f45a382a15d3eb5e7fe72076eec4ef0f56fde1adfd710866e729b9e5f3383d02202720a895914c69ab49359087364f06d337a2138305fbc19e20d18da78415ea9301",
				Encoders.Hex.EncodeData(sigParams.GetMultisigSignatures()[0].ToBytes()));
			...
			sigParams = PayToScriptHashTemplate.Instance.ExtractScriptSigParameters(new Script(scriptSig),
				sigParams.RedeemScript.PaymentScript);
			Assert.NotNull(sigParams);
			sigParams = PayToScriptHashTemplate.Instance.ExtractScriptSigParameters(new Script(scriptSig),
				new Script("OP_HASH160 b5b88dd9befc9236915fcdbb7fd50052df50c853 OP_EQUAL"));
			Assert.Null(sigParams);
			sigParams = PayToScriptHashTemplate.Instance.ExtractScriptSigParameters(new Script(scriptSig),
				new Script("OP_HASH160 OP_EQUAL"));
			Assert.Null(sigParams);
			///

			scriptSig =
				"0 0 51210364bd4b02a752798342ed91c681a48793bb1c0853cbcd0b978c55e53485b8e27c210364bd4b02a752798342ed91c681a48793bb1c0853cbcd0b978c55e53485b8e27d52ae";
		...
		public void P2PKHScriptSigShouldNotBeMistakenForP2SHScriptSig()
		{
			var p2pkhScriptSig =
				new Script(
					"304402206e3f2f829644ffe78b56ec8d0ea3715aee66e533a8195220bdea1526dc6ed3b202205eabcae791abfea55d54f8ec4e6de1bad1f7aa90e91687e81150b411e457025701 029f4485fddb359aeed82d71dc8df2fb0e83e31601c749d468ea92c99c13c5558b");
		...
		public void PayToMultiSigTemplateShouldAcceptNonKeyParameters()
		{
			var tx = Transaction.Parse(
				...
				Network.Main);
			var redeemScript = PayToScriptHashTemplate.Instance.ExtractScriptSigParameters(tx.Inputs[1].ScriptSig)
				.RedeemScript;
		...
		{
			var data = Encoders.Hex.DecodeData(
				"035c030441ef8fa580553f149a5422ba4b0038d160b07a28e6fe2e1041b940fe95b1553c040000000000000050db680300000000000002b0466f722050696572636520616e64205061756c");
		...
		public void script_CHECKMULTISIG23()
		{
			...
			keys = new[] {key2, key2}; // Can't re-use sig
			txTo23.Inputs[0].ScriptSig = sign_multisig(scriptPubKey23, keys, txTo23);
			AssertInvalidScript(scriptPubKey23, txTo23, 0, flags);

			keys = new[] {key2, key1}; // sigs must be in correct order
			txTo23.Inputs[0].ScriptSig = sign_multisig(scriptPubKey23, keys, txTo23);
			AssertInvalidScript(scriptPubKey23, txTo23, 0, flags);

			keys = new[] {key3, key2}; // sigs must be in correct order
			txTo23.Inputs[0].ScriptSig = sign_multisig(scriptPubKey23, keys, txTo23);
			AssertInvalidScript(scriptPubKey23, txTo23, 0, flags);

			keys = new[] {key4, key2}; // sigs must match pubkeys
			txTo23.Inputs[0].ScriptSig = sign_multisig(scriptPubKey23, keys, txTo23);
			AssertInvalidScript(scriptPubKey23, txTo23, 0, flags);

			keys = new[] {key1, key4}; // sigs must match pubkeys
			txTo23.Inputs[0].ScriptSig = sign_multisig(scriptPubKey23, keys, txTo23);
			AssertInvalidScript(scriptPubKey23, txTo23, 0, flags);
		...
		public void script_combineSigs()
		{
			...
			var hash1 = txTo.GetSignatureHash(scriptPubKey, 0, SigHash.All);
			var sig1 = new TransactionSignature(keys[0].Sign(hash1), SigHash.All);

			var hash2 = txTo.GetSignatureHash(scriptPubKey, 0, SigHash.None);
			var sig2 = new TransactionSignature(keys[1].Sign(hash2), SigHash.None);


			var hash3 = txTo.GetSignatureHash(scriptPubKey, 0, SigHash.Single);
			...
			var complete12 = new Script() + OpcodeType.OP_0 + Op.GetPushOp(sig1.ToBytes()) +
			                 Op.GetPushOp(sig2.ToBytes());
			var complete13 = new Script() + OpcodeType.OP_0 + Op.GetPushOp(sig1.ToBytes()) +
			                 Op.GetPushOp(sig3.ToBytes());
			var complete23 = new Script() + OpcodeType.OP_0 + Op.GetPushOp(sig2.ToBytes()) +
			                 Op.GetPushOp(sig3.ToBytes());
		...
		public void script_json_tests()
		{
			...
			foreach (var test in tests)
			{
				...
				if (test[i] is JArray)
				{
					...
					for (var ii = 0; ii < array.Count - 1; ii++)
					{
		...
		public void script_single_hashtype()
		{
			...
			var scriptPubKey =
				new Script("OP_DUP OP_HASH160 34fea2c5a75414fd945273ae2d029ce1f28dafcf OP_EQUALVERIFY OP_CHECKSIG");
			Assert.True(tx.Inputs.AsIndexedInputs().ToArray()[2].VerifyScript(scriptPubKey, out var error));
		}
