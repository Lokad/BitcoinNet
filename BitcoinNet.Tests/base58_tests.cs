	...
	public class base58_tests
	{
		...
		public void base58_keys_valid_gen()
		{
			...
			foreach (var test in tests)
			{
				...
				var metadata = test.GetDynamic(2);
				...
				else
				{
					...
					try
					{
						var addrOut = dest.GetAddress(network);
		...
		public void base58_keys_valid_parse()
		{
			...
			foreach (var test in tests)
			{
				...
				else
				{
					...
					Assert.True(addr is BitcoinScriptAddress == (exp_addrType == "script"),
		...
		public void ThrowOnInvalidECKey()
		{
			Assert.Throws<ArgumentException>(() =>
				new Key(Encoders.Base58.DecodeData("JEKNVnkbo3jma5nREBBJCD7MJVUPAg5THBwPPejEsG9v")));
		}
	}
}